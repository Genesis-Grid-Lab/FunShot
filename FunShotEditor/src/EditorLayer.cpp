...
namespace FS {
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(FS_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(FS_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".FunShot")
		{
			FS_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
