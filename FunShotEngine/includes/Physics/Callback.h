...
namespace Gen
{
    ...
    struct PxEventCallback : public PxSimulationEventCallback 
    {
        ...
        GEN_INLINE void onContact(const PxContactPairHeader& header, const 
        PxContactPair* pairs, PxU32 nbPairs) override  
        {
            ...
            GEN_DEBUG("onContact Event!");
        }
   
        // override the onTrigger callback
        GEN_INLINE void onTrigger(PxTriggerPair* pairs, PxU32 nbPairs) override  
        {
            ...
            GEN_DEBUG("onTrigger Event!");
        }
        
	    GEN_INLINE void onAdvance(const PxRigidBody*const* bodyBuffer, 
        const PxTransform* poseBuffer, const PxU32 count) override {}

        GEN_INLINE void onSleep(PxActor** actors, PxU32 count) 
        override { GEN_DEBUG("onSleep Event!"); }

        GEN_INLINE void onWake(PxActor** actors, PxU32 count) 
        override { GEN_DEBUG("onWake Event!"); }

        GEN_INLINE void onConstraintBreak(PxConstraintInfo* 
