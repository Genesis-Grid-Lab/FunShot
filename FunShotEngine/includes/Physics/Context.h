...
namespace Gen
{
    struct PhysicsContext
    {
        GEN_INLINE PhysicsContext() 
        {
            ...
            if (!m_Physics) 
            {
                GEN_ERROR("Error initializing PhysX m_Physics");
            ...
            if (!m_Scene) 
            {
                GEN_ERROR("Error creating PhysX m_Scene");
                m_Physics->release();
                m_Foundation->release();
                return;
            }   
        ...
        GEN_INLINE void AddRigidBody(Entity& entity)
        {                   
            ...
            if(entity.template Has<ColliderComponent>())
            {
                ...
                {
                    GEN_ERROR("Error creating collider invalid type provided");
                    return;
                }
            ...
            if (!body.Actor) 
            {
                GEN_ERROR("Error creating dynamic actor");               
                return;
            }
        ...
        GEN_INLINE void Simulate(uint32_t step, float dt)
        {
        ...
        GEN_INLINE void SetEventCallback(PxCallbackFunction&& callback)
        {
            m_EventCallback.m_Callback = callback;
        }

        GEN_INLINE PxConvexMeshGeometry CookMesh(const MeshData<ShadedVertex>& data) 
        {          
            // px vertex container
