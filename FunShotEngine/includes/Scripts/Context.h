...
namespace Gen
{
    ...
    {
        GEN_INLINE ScriptContext(EntityRegistry* scene, AppWindow* window)
        {       
            // import lua libraries
            m_Lua.open_libraries(sol::lib::base);

            // load FunShot api module
            m_Lua.require_file("Inputs", "Resources/Scripts/FunShotEngine/Input.lua");   
            m_Lua.require_file("Core", "Resources/Scripts/FunShotEngine/Core.lua");   
        ...
        GEN_INLINE bool AttachScript(Entity& entity, const std::string& name)
        {                    
            // check if handle is correct and has a constructor
            if (!m_Lua[name].valid() && !m_Lua[name]["Constructor"].valid()) 
            {
                GEN_ERROR("failed to create script: invalid script name!"); 
                return false;
            }
            ...
            if(!object.valid())
            {
                ...
                GEN_ERROR("failed to create script: {}", error.what());
                return false;
            }
        ...
        GEN_INLINE std::string LoadScript(const std::string& filepath)
        {
            ...
            {
                GEN_ERROR("failed to load script! core not initialized!");
                return "";
            }
            ...
            {
                GEN_ERROR("failed to load script: invalid file path!");
                return "";
            }
    ...
    private:
        ...
        GEN_INLINE void SetApiFunctions(EntityRegistry* scene, AppWindow* window)
        {
            ...
            (EntityID entity, const glm::vec3& force)
            {
                ...
                if(scene->all_of<RigidBodyComponent>(entity))
                {
                    ...
                    GEN_ERROR("trying to apply force to static body!");          
                    return false;
                }
                GEN_ERROR("trying to apply force to non physics entity!");          
                return false;
            });
