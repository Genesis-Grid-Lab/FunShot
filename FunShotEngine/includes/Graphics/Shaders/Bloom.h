...
namespace Gen
{
    ...
    { 
        GEN_INLINE BloomShader(const std::string& path, int32_t width, int32_t height): 
        Shader(path), m_Width(width), m_Height(height) 
        {
            ...
            for(auto i = 0; i < 2; i++)
            {
                ...
                GLCheck(glBindFramebuffer(GL_FRAMEBUFFER, m_FrameBuffer[i]);)
        ...
        GEN_INLINE void Compute(uint32_t brightnessMap, uint32_t stepCount)
        {
            // bind shader program
            ...
            {
                GLCheck(glBindFramebuffer(GL_FRAMEBUFFER, m_FrameBuffer[horizontal]);)
        ...
        GEN_INLINE uint32_t GetMap()
        {
            return m_PingPongMaps[0];
        }

        GEN_INLINE ~BloomShader()
        {
