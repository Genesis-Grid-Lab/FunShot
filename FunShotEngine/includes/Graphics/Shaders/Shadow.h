...
namespace Gen
{
    ...
    { 
        GEN_INLINE ShadowShader(const std::string& path): Shader(path) 
        {
            ...
            { 
                GEN_ERROR("CreateDepthBuffer() Failed!"); 
            }
        ...
        GEN_INLINE void Draw(Model3D& model, Transform3D& transform)
        {
        ...
        GEN_INLINE void BeginFrame(const glm::mat4& lightSpaceMtx)
        {
            // bind shadow shader
        ...
        GEN_INLINE uint32_t GetDepthMap()
        {
            return m_DepthMap;
        }
