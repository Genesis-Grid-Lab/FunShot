...
namespace Gen
{
    ...
    { 
        GEN_INLINE FinalShader(const std::string& filename, int32_t width, int32_t height): 
            Shader(filename) 
        {
        ...
        GEN_INLINE ~FinalShader()
        {
            glDeleteTextures(1, &m_Final); 
            glDeleteFramebuffers(1, &m_FBO); 
        }

        GEN_INLINE void Render(uint32_t map, uint32_t bloom, bool useFBO) 
        {
    ...
    private:
        GEN_INLINE void CreateBuffer(int32_t width, int32_t height) 
        {
            // create frame buffer
