...
namespace Gen
{
    struct Shader 
    {
        GEN_INLINE Shader(const std::string& filename) 
        {
            m_ShaderID = Load(filename);
        }

        GEN_INLINE virtual ~Shader() 
        {
            glDeleteProgram(m_ShaderID);
        }

        GEN_INLINE void Unbind() 
        { 
            glUseProgram(0); 
        } 

        GEN_INLINE void Bind() 
        { 
            glUseProgram(m_ShaderID); 
        }  
    
    private:
        GEN_INLINE uint32_t Build(const char* src, uint32_t type) 
        {
        ...
        GEN_INLINE uint32_t Link(uint32_t vert, uint32_t frag) 
        {            
        ...
        GEN_INLINE uint32_t Load(const std::string& filename) 
        {
            std::ifstream fs;
            ...
            {	
                GEN_ERROR("Load('{}') Failed: {}", filename, e.what());	
            }
            return 0;
        }  
