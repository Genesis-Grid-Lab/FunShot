...
namespace Gen
{
	...
	struct Model 
	{
		GEN_INLINE virtual JointMatrices* Animate(float) { return nullptr; }
		GEN_INLINE virtual bool HasJoints() { return false; }
		GEN_INLINE virtual void Load(const std::string&) {}
		GEN_INLINE virtual void Draw(uint32_t) {}
	};	
    ...
    {
        GEN_INLINE StaticModel() = default;

        GEN_INLINE StaticModel(const std::string& path)
        {
            Load(path);
        }
		
		GEN_INLINE void Draw(uint32_t mode) override final
        {
			// render meshes
        ...
        GEN_INLINE void Load(const std::string& path) override final
        {
			...
			if (!ai_scene || ai_scene->mFlags == AI_SCENE_FLAGS_INCOMPLETE || !ai_scene->mRootNode) {
				GEN_ERROR("failed to load model-{}", importer.GetErrorString());
				return;
			}
    ...
    private:
		GEN_INLINE void ParseMesh(aiMesh* ai_mesh) 
        {
			// mesh data
		...
		GEN_INLINE void ParseNode(const aiScene* ai_scene, aiNode* ai_node) 
        {
	...
	struct SkeletalModel : Model
	{
		...
		GEN_INLINE SkeletalModel() = default;						

		GEN_INLINE SkeletalModel(const std::string& path)
		{
			Load(path);
		}
		
		GEN_INLINE bool HasJoints()  override final 
		{ 
			return m_JointCount; 
		}
		...
		GEN_INLINE JointMatrices* Animate(float dt) override final
		{
			return m_Animator->Animate(dt);
		}

	private:
		GEN_INLINE void ParseNode(const aiScene* ai_scene, aiNode* ai_node, JointMap& jointMap) 
        {
			for (uint32_t i = 0; i < ai_node->mNumMeshes; i++) 
            {
		...
		GEN_INLINE void SetVertexJoint(SkeletalVertex& vertex, int32_t id, float weight) 
		{
		...
		GEN_INLINE void ParseHierarchy(aiNode* ai_node, Joint& joint, JointMap& jointMap)
		{
		...
		GEN_INLINE void ParseAnimations(const aiScene* ai_scene, JointMap& jointMap) 
        {
		...
		GEN_INLINE void ParseMesh(const aiMesh* ai_mesh, JointMap& jointMap) 
        {
			// mesh data
